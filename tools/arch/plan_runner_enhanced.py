...
def execute_task(task_node: TaskNode, dag: ExecutionDAG, tracer: ExecutionTracer, 
...
def run_plan_dag_aware(plan_path: Path) -> bool:
    """
...
# Main plan runner logic (backwards compatibility)
def run_plan(plan_path: Path):
    """Legacy plan runner function for backwards compatibility."""
    return run_plan_dag_aware(plan_path)

if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python plan_runner_enhanced.py <plan_path>")
        print("Example: python plan_runner_enhanced.py plans/sample-plan-001.yaml")
        sys.exit(1)
    
    success = run_plan_dag_aware(Path(sys.argv[1]))
